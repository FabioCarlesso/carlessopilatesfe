...
> Os tokens `--surface` e `--c-primary`, usados pelos cards das listagens de reavaliações, sessões, NFSEs e planos de tratamento, **não existem** em `src/styles/_tokens.scss`: nessas telas o fundo do card não é pintado e a borda de destaque cai para `currentColor` (uma faixa creme de 4px no tema escuro). Em card novo, use `--bg-elev` para a superfície elevada e `--text-brand` para o destaque — é o único token de marca com versão própria no tema escuro.
...
A navbar identifica o usuário autenticado no formato `Nome · Perfil` (ex.: `Fabio Carlesso · Administrador`), antes do link **Alterar senha**, em todas as telas protegidas. O rótulo do perfil vem do `ROLE_LABEL` exportado por `src/app/core/models/usuario-admin.ts` — derivado de `ROLE_OPTIONS` e compartilhado com a listagem administrativa de usuários, para não haver duas fontes de verdade para "Administrador"/"Usuário". O valor é resolvido uma vez na criação do `AppComponent` e a cada navegação concluída (login e logout sempre navegam), e não na interpolação do template, porque `getCurrentUser()` faz `JSON.parse` + validação a cada chamada e o template da raiz reavalia a cada ciclo de detecção de mudanças. Quando há token mas o `currentUser` está ausente ou corrompido no `localStorage`, a navbar renderiza normalmente e apenas omite a identificação. O elemento não é botão nem link: é um `<span>` com `title` contendo o texto completo, já que no desktop nomes longos são truncados com reticências para não empurrar os botões de ação; em `≤1024px` a identificação abre o bloco de ações do painel expandido, ocupando a largura total sem o alvo de toque de 44px dos botões vizinhos. Ela é o primeiro filho de `.navbar-actions` (antes da busca global e, portanto, do link **Alterar senha**), de modo que a ordem visual e a ordem de leitura coincidem nos dois breakpoints, sem recorrer a `order`. A cor é a **literal** `#f0ede8`, e não `--c-cloud-dancer`: esse token é re-tematizado em `[data-theme="dark"]` (`#f0ede8` → `#0e1620`) e sobre o fundo fixo `--c-horizonte` da navbar renderia 2,16:1 no tema escuro, contra 7,20:1 da literal nos dois temas — é a mesma razão pela qual os links do menu usam cores literais (comentário em `styles.scss:100`). O `currentUser` é gravado apenas no login, então uma alteração do próprio nome via `/admin` só aparece no login seguinte.
