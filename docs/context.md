...
35. Ajuste do `AuthService` para armazenar dados do usuário logado (`nome`, `email`, `perfil`) em `localStorage` via chave `usuarioLogado`, com método `getUsuario()` para recuperação e limpeza automática no logout
...
### Autenticação JWT
O frontend implementa autenticação stateless via JWT consumindo `POST /api/auth/login`. O token é armazenado em `localStorage` pela `AuthService` sob a chave `accessToken`. Além do token, o `AuthService` armazena os dados do usuário logado (`nome`, `email`, `perfil`) como JSON na chave `usuarioLogado`. O método `getUsuario()` recupera esses dados tipados como `UsuarioLogado`; retorna `null` quando não há sessão ou o JSON armazenado está corrompido. O `authInterceptor` (functional interceptor) injeta o header `Authorization: Bearer <token>` em todas as requisições, exceto no próprio endpoint de login. Quando uma requisição autenticada retorna `401`, o interceptor executa logout, remove o token e os dados do usuário e redireciona para `/login`, cobrindo o fluxo de token expirado retornado pelo Spring Security. O `authGuard` (functional guard) protege todas as rotas autenticadas e redireciona para `/login` quando não há token. O `AppComponent` exibe a navbar e o botão **Sair** apenas quando o usuário está autenticado. O logout remove o token e os dados do usuário e redireciona para `/login`. Controle de acesso por perfil e tela dedicada para `403` ainda são pendências futuras.
