...
A tela `/pacientes/:pacienteId/avaliacao-fisioterapeutica` usa Reactive Forms para criar ou editar a avaliação fisioterapêutica do paciente. O componente valida o parâmetro `pacienteId` com `parseRouteNumberParam`, carrega a identificação do paciente por `PacienteService.buscar` e tenta carregar a avaliação via `GET /api/avaliacoes-fisioterapeuticas/paciente/{pacienteId}`. Retorno `404` indica ausência de avaliação e mantém o formulário em modo de cadastro. O salvamento usa `POST /api/avaliacoes-fisioterapeuticas` quando ainda não há registro e `PUT /api/avaliacoes-fisioterapeuticas/{id}` quando já existe. Os campos `queixaPrincipal` e `objetivosTratamento` são obrigatórios e rejeitam conteúdo somente com espaços. O campo `escalaDor` aceita valores opcionais entre 0 e 10. Demais campos — `historicoClinico`, `examePostural`, `exameFisico`, `testesEspeciais`, `localizacaoDor`, `hipoteseDiagnostica`, `condutaTerapeutica` e `observacoes` — são opcionais.
