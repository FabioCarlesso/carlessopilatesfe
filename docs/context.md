...
A rota `/pacientes/:pacienteId/nfse-emitidas/nova` (`PacienteNfseEmitidaFormComponent`) registra ou atualiza a NFSE emitida via `POST /api/api/nfse-emitidas`. Os campos obrigatórios são `competencia` (formato `MM/AAAA`, mês entre `01` e `12`) e `dataEmissao`; os opcionais `numeroNota` (máx. 60 caracteres), `valor` (≥ 0) e `observacoes` são sanitizados — strings vazias viram `null` e o valor é convertido para número ou `null`. Após sucesso o formulário navega para a listagem. O `NfseEmitidaService` usa o prefixo `/api/api/nfse-emitidas` pelo mesmo motivo do `RelatorioService`: o backend expõe o recurso em `/api/nfse-emitidas` e o proxy de desenvolvimento e o Nginx removem o primeiro prefixo `/api`. A tela de detalhe do paciente oferece o card **NFSEs Emitidas** para acesso rápido.
