...
describe('PacienteNfseEmitidaListComponent', () => {
  ...
  // Os cards usavam `--surface`, `--c-primary` e `--radius-lg`, que nunca foram
  ...
  // (issue #213). O guard trava os nomes que existem de fato, no card da listagem
  // e no destaque da última nota, que é quem tem a borda esquerda.
  it('should paint the cards with real tokens in both themes', async () => {
    await setup([mockNota]);
    ...
    try {
      ...
      const card = el.querySelector('.nfse-card') as HTMLElement;
      const destaque = el.querySelector('.ultima-nota') as HTMLElement;

      expect(getComputedStyle(card).borderRadius).toBe('8px');
      expect(getComputedStyle(destaque).borderRadius).toBe('8px');

      document.documentElement.setAttribute('data-theme', 'light');
      expect(getComputedStyle(card).backgroundColor).toBe('rgb(255, 255, 255)');
      expect(getComputedStyle(destaque).backgroundColor).toBe('rgb(255, 255, 255)');
      expect(getComputedStyle(destaque).borderLeftColor).toBe('rgb(55, 79, 108)');

      document.documentElement.setAttribute('data-theme', 'dark');
      expect(getComputedStyle(card).backgroundColor).toBe('rgb(24, 34, 48)');
      expect(getComputedStyle(destaque).backgroundColor).toBe('rgb(24, 34, 48)');
      expect(getComputedStyle(destaque).borderLeftColor).toBe('rgb(168, 188, 202)');
    } finally {
